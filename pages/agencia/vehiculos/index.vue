  ...
  <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
    ...
    <div class="sm:flex sm:justify-between sm:items-center mb-8">
      ...
      <button  @click.stop="createModalOpen = true" class="btn bg-indigo-500 hover:bg-indigo-600 text-white">
        ...
        <span class="hidden xs:block ml-2">Agregar Vehiculo</span>
      </button>
    </div>
    ...
    @loading-change="fetchData()"
     ...
     :total="totalResults"
     v-else-if="data.length > 1"></VehiclesTable>
...
import type { VehiculoModel } from "~~/server/model/vehiculo";
...
const data = ref<VehiculoModel[]>([]);
const totalResults = ref(0);
const errors = ref([]);
const fetchData = async () => {
  try {
    loading.value = true;
    const result = await $fetch("/api/vehiculo");
    data.value = result.data as VehiculoModel[];
